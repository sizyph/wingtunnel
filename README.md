...
Every feature — all five views, particles, forces, Cp/wake, free flight, optimizer, polar, MRT, thermal, free-slip walls, and the Kármán tone — runs on both paths.
...
- **Channel walls**: no-slip by default, or **free-slip** (specular reflection) via toggle — removes the ~10 % blockage-induced Cl inflation.
...
- **Kármán tone**: a DFT of the Cl history maps the dominant shedding frequency to an audible WebAudio sine.
